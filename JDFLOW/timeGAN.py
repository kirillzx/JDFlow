...
def timeGAN_train(data, n_seq, train_steps):
    ...
    # synth_data_reduced = np.vstack(synthetic_sample)[:len(data[0])]
    synth_data_reduced = np.vstack([np.array(synthetic_sample)[:, 0, :], synthetic_sample[-1][1:]])
    # stock_data_reduced = np.vstack(real_sample)[:len(data[0])]
